...
{   protected $request;
    ...
    public function activationCode($token)
    {
        ...
        if(!$user)
        {
            return redirect()->route('login')->with('danger', 'This token doesn\'t exist match any user.');
        }
   ...
   return back()->with('success', 'You have just resend the new activation code.');
    ...
    public function ActivationAccountChangeEmail($token)
    {
        ...
        if($this->request->isMethod('post'))
        {
            ...
            {
                return back()->with([
                    'danger' => 'This address email is already used! Please entre anbother email address!',
                    'new_email' => $new_email
                ]);
            }
            else
